...
export const GetPhotos = createAsyncThunk("photos/getPhotos", async (pageInfo) => {
    ...
    try {
        ...
        if (!pageInfo.query == "") {
            ...
            console.log(`https://api.unsplash.com/search/photos?query=${pageInfo.query}&per_page=20&page=${pageInfo.page}&client_id=${UNSPLASH_ACCESS_KEY}`)
        } else {
            req = await fetch(`https://api.unsplash.com/photos?per_page=20&page=${pageInfo.page}&client_id=${UNSPLASH_ACCESS_KEY}`)
            console.log(`https://api.unsplash.com/photos?per_page=20&page=${pageInfo.page}&client_id=${UNSPLASH_ACCESS_KEY}`)
        }
        if (req.ok) {
